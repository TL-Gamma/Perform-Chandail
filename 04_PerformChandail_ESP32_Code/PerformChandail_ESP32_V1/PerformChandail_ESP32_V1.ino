...
#include <cmath>                // Bibliothèque pour les fonctions mathématiques comme sin, cos, sqrt, etc.
...
#define DEFAULT_FILE "/gps_data.json"  // Nom par défault du fichier de sauvegarde de la course
...
public:
  ...
  void calculatePositiveAltitudeChange(float altitudeGPS) {
    ...
    if (altitudeInitialized) {
      // Si l'altitude GPS est supérieure à l'altitude précédente
      ...
      // Si c'est la première mesure, initialiser l'altitude précédente
  ...
  void calculateTotalDistance(float latitude, float longitude) {
    if (distanceInitialized) {
      float distance = calculateDistance(previousLatitude, previousLongitude, latitude, longitude);
  ...
  String getDuration() {
    ...
    snprintf(buffer, sizeof(buffer), "%02d:%02d:%02d", hours, minutes, seconds);  // Concatenation de la chaine de caractères
...
private:
  // Variables durée
  unsigned long startTime = 0;  // Variables de début de course
  ...
  bool altitudeInitialized;            // Marquer si l'altitude initiale a été initialisée
};

GPSData gpsData; // Declare a global instance of the GPSData class
...
// Fonction de blink de la led RGB
void ledBlink(uint8_t redValue, uint8_t greenValue, uint8_t blueValue) {
...
void getGPSData() {
  ...
  if (gps.speed.isValid()) {
    gpsData.calculateAverageSpeed(gps.speed.kmph());
...
void stopRecording() {
  ...
  if (dataFile) {
    ...
    dataFile.println("\t\"total_denivle\": \"" + String(gpsData.getPositiveAltitudeChange()) + "\",");
