...
void stopRecording() {
  File dataFile = SD.open(filename, FILE_APPEND);
  if (dataFile) {
    // Assigner les valeurs dans l'objet JSON
    dataJsonDoc["date"] = getDate();
    dataJsonDoc["duration"] = gpsData.getDuration();
    dataJsonDoc["average_speed"] = gpsData.getAverageSpeed();
    dataJsonDoc["denivle_positif"] = gpsData.getPositiveAltitudeChange();
    dataJsonDoc["total_distence"] = gpsData.getTotalDistance();

    // Sérialiser l'objet JSON
    String jsonDataString;
    serializeJson(dataJsonDoc, jsonDataString);

    // Écrire les données dans le fichier JSON
    dataFile.println(jsonDataString);
    dataFile.println("]");
    dataFile.close();
    ledBlink(GREEN);
