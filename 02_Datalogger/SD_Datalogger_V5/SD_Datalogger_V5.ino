...
#define INTERVALLE_ACQUISITION 5000  // Intervalle entre chaque acquisition de données GPS (en millisecondes)
...
#define EEPROM_ADDRESS 0             // Adresse de début de l'EEPROM pour stocker le nom du fichier
#define MAX_FILENAME_LENGTH 512      // Longueur maximale du nom de fichier à stocker
...
void createNewFile(const char *chemin) {
  ...
  // Sauvegarde du nom du fichier dans l'EEPROM
  EEPROM_writeString(EEPROM_ADDRESS, filename);
...
void EEPROM_init() {
  // Initialisation de l'EEPROM avec la taille correspondant à la longueur maximale du nom de fichier
  if (!EEPROM.begin(MAX_FILENAME_LENGTH)) {
    Serial.println("Erreur lors de l'initialisation de l'EEPROM !");
    // Ajoutez ici la gestion de l'erreur, par exemple, une boucle infinie ou un redémarrage du système
  } else {
    Serial.println("EEPROM initialisée avec succès.");
  }
}
...
void EEPROM_writeString(int addr, const String &str) {
  ...
  Serial.println(EEPROM_readString(EEPROM_ADDRESS));
  lcd.setCursor(0, 1);
  lcd.print("FW:");
  lcd.print(EEPROM_readString(EEPROM_ADDRESS));
}

void EEPROM_findLastFile() {
  // Lecture du nom du dernier fichier depuis l'EEPROM


  String lastFilename = EEPROM_readString(EEPROM_ADDRESS);
  Serial.print("EEPROM Find last file :");
  Serial.print(lastFilename);
  lcd.setCursor(0, 1);
  lcd.print("LF:");
  lcd.print(lastFilename);
  if (lastFilename != "") {
    // Ouvrir le fichier sur la carte SD en mode écriture
    File lastFile = SD.open(lastFilename, FILE_APPEND);

    if (lastFile) {
      // Écrire les données dans le fichier
      lastFile.print("{}]");

      // Fermer le fichier après les modifications
      lastFile.close();

      Serial.print("Last file found and '{}]' added: ");
      Serial.println(lastFilename);
      ledBlink(YELLOW);
      return;  // Sortir de la fonction une fois le traitement terminé avec succès
    } else {
      Serial.println("Erreur : impossible d'ouvrir le fichier");
    }
  } else {
    // Si aucun fichier n'est trouvé ou s'il y a une erreur lors de l'ouverture du fichier
    Serial.println("Erreur : fichier non trouvé");
    ledBlink(MAGENTA);
  }
}
String EEPROM_readString(int addr) {
...
void EEPROM_reset() {
  ...
  // Effacement de chaque adresse de l'EEPROM en écrivant des zéros
  for (int i = 0; i < EEPROM.length(); ++i) {
    EEPROM.write(i, 0);
  }

  // Fin de l'utilisation de l'EEPROM
  EEPROM.end();
}
